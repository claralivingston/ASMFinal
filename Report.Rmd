...
```{r}
#require(gridExtra)
#plot1 <- ggplot(AllData, aes(x=Attendence,y = WinPct)) + geom_point() + facet_wrap(~Type) +geom_smooth(method=lm)

#plot2 <- ggplot(AllData, aes(x=Capacity,y = WinPct)) + geom_point() + facet_wrap(~Type)+geom_smooth(method=lm)

#plot3 <- ggplot(AllData, aes(x=Population,y = WinPct)) + geom_point() + facet_wrap(~as.factor(Happiness)) +geom_smooth(method=lm)

#plot4 <- ggplot(AllData, aes(x=Attendence,y = WinPct)) + geom_point() + facet_wrap(~CapFactor) +geom_smooth(method=lm)

#grid.arrange(plot1, plot2,ncol=2)
#grid.arrange(plot3,plot4,ncol=2)
```

- discussion of choosing to add type/sport as a variable rather than random effect
- fixed effects and why we chose the interactions we did, with support from eda plots
- discuss why we ended up not using the interactions we thought might be influential

...
AllData <- AllData%>% mutate(Capacity2=scale(Capacity),Population2=scale(Population),FranchiseValue2=scale(FranchiseValue))

sports.glmer <- glmer(WinPct~Capacity2+Attendence+Type+FranchiseValue2+Year.opened+Capacity2:Attendence+Type:FranchiseValue2+Year.opened:Type+Type:Attendence+FranchiseValue2:Attendence+Type:Capacity2+(Population2+Trump.Vote+Happiness)^2+Happiness:Capacity2+Trump.Vote:Attendence+Happiness:Type+Population2:Year.opened+Happiness:Year.opened+(1|State),family=binomial,data=AllData,weights=HomeGames)
